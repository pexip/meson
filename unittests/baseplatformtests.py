...
class BasePlatformTests(TestCase):
    ...
    def setUp(self):
        ...
        ) = get_backend_commands(self.backend)
        # Test directories
    ...
    def init(
        ...
        *,
        ...
        if inprocess:
            ...
            try:
                out = self._run(
                    self.setup_command + args + extra_args + build_and_src_dir_args,
    ...
    def build(
        self, target=None, *, extra_args=None, override_envvars=None, stderr=True
    ):
        if extra_args is None:
            extra_args = []
        ...
        return self._run(
            self.build_command + args + extra_args,
    ...
    def get_compdb(self):
        if self.backend is not Backend.ninja:
            raise SkipTest(
                f"Compiler db not available with {self.backend.name} backend"
            )
        try:
            with open(
                os.path.join(self.builddir, "compile_commands.json"), encoding="utf-8"
            ) as ifile:
    ...
    def get_meson_log_compiler_checks(self):
        ...
        with self._open_meson_log() as log:
            cmds = [
                split_args(l[len(prefix) : -len(suffix)])
                for l in log
    ...
    def introspect(self, args):
        ...
        out = subprocess.check_output(
            self.mintro_command + args + [self.builddir], universal_newlines=True
        )
    ...
    def introspect_directory(self, directory, args):
        if isinstance(args, str):
            args = [args]
        out = subprocess.check_output(
            self.mintro_command + args + [directory], universal_newlines=True
        )
        try:
            obj = json.loads(out)
        except Exception as e:
